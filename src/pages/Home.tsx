...
try {
    const agentManager = await DidSdk.createAgentManager(D_ID_AGENT_ID, {
        ...
        // Fix: Removed 'language' and 'tts' properties from AgentManagerOptions to resolve TypeScript errors as they are not known properties.
        // LLM generation language is now handled via instructions.
    ...
    try {
      ...
      if (!hasKey) {
          try {
            ...
            // We assume success here as per guidelines to handle race conditions.
            // Do not check for return value as it might be void.
          } catch (keyError: any) {
      ...
      while (!operation.done) {
          ...
          // Fix: Use the correct getVideosOperation method as per @google/genai guidelines.
...
--- 文字起こし終了 ---`;
  ...
  // Moved useEffect from top level to here to avoid hoisting issues with hangUp
  useEffect(() => {
    const greetingText = 'はじめまして、わたくしリファスタの「AI査定員」です。\nご売却を検討されているお品物について、お気軽にご質問ください。\n画像を添付していただくと、より詳しいご案内が可能です。';
                            ...
                            <Typography variant="caption" color="text.secondary" display="block" gutterBottom>
                             ...
                             <Stack direction="row" spacing={2} sx={{ mt: 2, mb: 2 }}>
                                <FormControl fullWidth>
                                    {/* Fix: Explicitly passing children as a prop to satisfy strict TypeScript requirements in some environments */}
                                    <InputLabel id="voice-lang-select-label" children="言語" />
                                    <Select
                                ...
                                <FormControl fullWidth disabled={!editableSettings.voice_lang || filteredVoices.length === 0}>
                                    {/* Fix: Explicitly passing children as a prop to satisfy strict TypeScript requirements in some environments */}
                                    <InputLabel id="voice-name-select-label" children="音声 (男性/女性など)" />
                                    <Select
